...
handleHeaderAction(event) {
    ...
    // Update the checked status of the actions
    cols.find(col => col.label === colDef.label).actions.forEach(action => {
        action.checked = action.name === actionName;
    });

    // Update the columns with the modified actions
    this.columns = cols;

    if (actionName !== 'all') {
        // Filter the cases based on the selected action
        this.cases = this.ALL_CASES.filter(_case => _case.brand === actionName);
    } else {
        // Display all cases if 'All' is selected
