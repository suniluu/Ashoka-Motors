...
export default class Testdependentpicklist extends LightningElement {
  ...
  wiredAccounts({ error, data }) {
    if (data) {
      this.records = data.map(account => ({ ...account, isSelected: false }));;
    }
  }
  @track options1 = [];
    ...
    getallvehciles1({
      ...
      .then(result => {
        ...
        this.records = result.map(record => ({
          ...record,
          isSelected: this.selectedIds.has(record.Id) ? true : false
        }));
      })
      .catch(error => {
        this.errors = error;
  ...
  handleAccountClick(event) {
    ...
    this.records = this.records.map(account => ({
      ...account,
      isSelected: account.Id === this.recordId ? !account.isSelected : account.isSelected
    }));
