...
export default class Testdependentpicklist extends NavigationMixin(LightningElement) {
  @api customerid;
  @track renault=Renault;
  @track audi= Audi ;
  @track honda=HONDA;
  @track toyota=Toyota;
  @track mahindra= Mahindra;
  @track bmw=BMW ;
  @track marutisuzuki=MarutiSuzuki;
  @track tata=Tata;
  @track kia=Kia;
  @track volswagen=Volkswagen;
  @track ford=Ford;
  @track hyundai=Hyundai;
  ...
  handlesearch() 
  {
    ...
    .then(result => {
      ...
      this.records = result.map(record => ({
        ...record,
        isSelected:'https://trainingccx-dev-ed--c.develop.vf.force.com/resource/1683704904000/MahindraNew',
      }));
    })
    .catch(error => {
      this.errors = error;
